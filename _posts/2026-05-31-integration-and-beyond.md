...
* **부정적분 (Antiderivative / Indefinite Integral):** 도함수가 $f(x)$인 함수 $F(x)$를 역도함수(Antiderivative)라고 부르며, $F(x) = \int f(x)dx$ 로 표기합니다.
* **미적분학의 기본 정리 (Fundamental Theorem of Calculus):** 함수 $f(x)$가 연속일 때, 구간 $[a, b]$에서 곡선 아래의 면적(정적분)은 다음과 같이 계산됩니다.
  $$ \int_{a}^{b} f(x)dx \equiv F(x)\Big|_{a}^{b} \equiv F(b) - F(a) $$
* **주요 부정적분 공식 (Well-known indefinite integrals):** * $\int x^n dx = \frac{x^{n+1}}{n+1} + C \quad (n \ne -1)$
...
* **정적분의 기본 성질:** 구간이 같으면 $\int_{a}^{a} f(x)dx = 0$, 구간이 역전되면 $\int_{a}^{b} f(x)dx = -\int_{b}^{a} f(x)dx$ 이며, 적분 구간을 나눌 수 있습니다 ($\int_{a}^{b} = \int_{a}^{c} + \int_{c}^{b}$).
* **합의 법칙:** $\int [f(x) + g(x)]dx = \int f(x)dx + \int g(x)dx$
* **부분적분 (Integration by parts):** $\int f(x)g'(x)dx = f(x)g(x) - \int f'(x)g(x)dx$
* **치환적분 (Substitution rule):** $u = g(x)$ 로 치환할 때, $\int f(g(x))g'(x)dx = \int f(u)du$
...
* **테일러 급수 (Taylor Series):** 임의의 차수 $k$에 대한 도함수를 $f^{(k)}(x)$라 할 때, 점 $a$ 주변에서 $f(x)$의 전개는 다음과 같습니다.
  $$ f(x) = \sum_{k=0}^{\infty} \frac{f^{(k)}(a)}{k!} (x-a)^k $$
* **매클로린 급수 (Maclaurin Series):** $a=0$일 때의 테일러 급수입니다.
...
* **필수 수열의 합 (Miscellaneous Sums):**
  ...
  * 무한 등비급수: $\sum_{k=0}^{\infty} p^k = \frac{1}{1-p} \quad (-1 < p < 1)$
...
* **로피탈의 정리 (L'Hospital's Rule):** 극한에서 $\frac{0}{0}$ 이나 $\frac{\infty}{\infty}$ 형태의 부정형(Indeterminate ratios)이 발생할 때 사용합니다. $\lim_{x \to a} f(x)$와 $\lim_{x \to a} g(x)$가 모두 $0$ 이거나 $\infty$로 향할 때, 다음이 성립합니다.
  $$ \lim_{x \to a} \frac{f(x)}{g(x)} = \lim_{x \to a} \frac{f'(x)}{g'(x)} $$
* **이중 적분 (Double Integration):** 단일 적분이 곡선 아래의 '면적(Area)'을 구하는 것이라면, 이중 적분은 3차원 함수 $f(x, y)$ 아래의 '부피(Volume)'를 나타냅니다. 계산 시 적분의 순서(order of integration)를 바꾸어 계산($dxdy \leftrightarrow dydx$)해도 동일한 결과를 얻을 수 있습니다.
