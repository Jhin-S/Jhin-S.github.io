...
* **전체 집합과 공집합:** 다루는 모든 대상을 포함하는 전체 집합(Universal set)은 $U$로, 아무런 원소가 없는 공집합(Empty set)은 $\emptyset$으로 표기합니다.
...
* **부분집합 (Subset) 및 상등:** 집합 $A$의 모든 원소가 집합 $B$의 원소일 때, $A$는 $B$의 부분집합($A \subseteq B$)이 됩니다. 만약 $A \subseteq B$ 이면서 동시에 $B \subseteq A$ 라면 두 집합은 같습니다($A=B$).
* **여집합 (Complement):** 전체 집합 $U$에 속하면서 $A$에는 속하지 않는 원소들의 집합을 $\overline{A}$로 표기합니다.
* **교집합(Intersection)과 합집합(Union):** 두 집합에 공통으로 속하는 원소의 집합은 $A \cap B$이며, 적어도 한 집합에 속하는 원소의 집합은 $A \cup B$입니다. 만약 공통 원소가 하나도 없다면($A \cap B = \emptyset$), 두 집합은 서로소(Disjoint 혹은 Mutually exclusive)라고 부릅니다.
* **차집합 (Difference):** $A$에는 속하지만 $B$에는 속하지 않는 집합으로, $A - B \equiv A \cap \overline{B}$ 로 정의됩니다.
* **대칭 차집합 (Symmetric Difference / XOR):** 두 집합 중 어느 한 쪽에만 속하는 원소들의 집합으로, $A \Delta B \equiv (A - B) \cup (B - A) = (A \cup B) - (A \cap B)$ 로 정의됩니다.
...
1. **여집합의 법칙 (Complement Laws):** $A \cup \overline{A} = U$, $A \cap \overline{A} = \emptyset$, $\overline{\overline{A}} = A$
2. **교환 법칙 (Commutative):** $A \cup B = B \cup A$, $A \cap B = B \cap A$
3. **결합 법칙 (Associative):** $A \cup (B \cup C) = (A \cup B) \cup C$, $A \cap (B \cap C) = (A \cap B) \cap C$
4. **분배 법칙 (Distributive):** $A \cup (B \cap C) = (A \cup B) \cap (A \cup C)$, $A \cap (B \cup C) = (A \cap B) \cup (A \cap C)$
5. **드 모르간의 법칙 (DeMorgan's Laws):** $\overline{A \cup B} = \overline{A} \cap \overline{B}$, $\overline{A \cap B} = \overline{A} \cup \overline{B}$
